...
#index_dir = "/Users/cenkayberkin/Desktop/PA12/toy_example/outDir"
...
def read_posting(termId):
    ...
    termInformation = file_pos_dic[str(termId)]
    index_f.seek(termInformation[0])
    line = index_f.read(termInformation[1])
    parts =  line.split("|")
    termId = int(parts[0])
    df = int(parts[1])
    
    postings = parts[2].rstrip('\n')
    decodedPostings = Compress.vb_decode(postings)
    gappedDecodedPostings = Compress.fromGaps(decodedPostings)
    return (termId,df,gappedDecodedPostings)
